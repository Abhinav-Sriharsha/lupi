...
export type VoiceOrbVariant = "default" | "blue" | "violet" | "emerald" | "amber";
...
export type OrbWrapState = "asleep" | "idle" | "listening" | "thinking" | "speaking";

export function deriveOrbWrapState(
  voiceState: ReturnType<typeof useVoiceState>,
): OrbWrapState {
  if (!voiceState || voiceState.status.type === "ended") return "asleep";
  if (voiceState.status.type === "starting") return "thinking";
  if (voiceState.isMuted) return "idle";
  if (voiceState.mode === "speaking") return "speaking";
  return "listening";
}
...
export const VoiceOrb: FC<VoiceOrbProps> = memo(
  ({ state: stateProp, variant = "default", className }) => {
    ...
    const render = useCallback(() => {
      ...
      gl.uniform3fv(uniforms.u_color0, colorsRef.current[0]!);
      gl.uniform3fv(uniforms.u_color1, colorsRef.current[1]!);
      gl.uniform3fv(uniforms.u_color2, colorsRef.current[2]!);
